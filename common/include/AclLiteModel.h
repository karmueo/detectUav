...
class AclLiteModel
{
  public:
     ...
     * Other: Failed to create
     */
    AclLiteError CreateInput(void    *input1,
                             uint32_t input1size,
                             void    *input2,
                             uint32_t input2size);
    /**
    ...
    AclLiteError
         GetModelOutputInfo(std::vector<ModelOutputInfo> &modelOutputInfo);
    void DestroyInput();
  ...
  private:
    ...
    AclLiteError
    AddDatasetBuffer(aclmdlDataset *dataset, void *buffer, uint32_t bufferSize);
    AclLiteError
         GetOutputItem(InferenceOutput &out, uint32_t idx, bool isDevice);
    void Unload();
    void DestroyDesc();
    void DestroyOutput();
