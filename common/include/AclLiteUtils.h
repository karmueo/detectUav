 ...
 * @return bytes size of image
 */
#define RGBF32_IMAGE_SIZE(width, height)                                       \
...
#define SHARED_PTR_DVPP_BUF(buf)                                               \
    (shared_ptr<uint8_t>((uint8_t *)(buf), [](uint8_t *p) { acldvppFree(p); }))
...
#define SHARED_PTR_DEV_BUF(buf)                                                \
    (shared_ptr<uint8_t>((uint8_t *)(buf), [](uint8_t *p) { aclrtFree(p); }))
...
#define SHARED_PTR_U8_BUF(buf)                                                 \
    (shared_ptr<uint8_t>((uint8_t *)(buf), [](uint8_t *p) { delete[] (p); }))
...
#define ACLLITE_LOG_ERROR(fmt, ...)                                            \
    ...
    {                                                                          \
        aclAppLog(                                                             \
            ACL_ERROR, __FUNCTION__, __FILE__, __LINE__, fmt, ##__VA_ARGS__);  \
...
#define ACLLITE_LOG_INFO(fmt, ...)                                             \
    ...
    {                                                                          \
        aclAppLog(                                                             \
            ACL_INFO, __FUNCTION__, __FILE__, __LINE__, fmt, ##__VA_ARGS__);   \
...
#define ACLLITE_LOG_DEBUG(fmt, ...)                                            \
    ...
    {                                                                          \
        aclAppLog(                                                             \
            ACL_DEBUG, __FUNCTION__, __FILE__, __LINE__, fmt, ##__VA_ARGS__);  \
        fprintf(stdout, "[INFO]  " fmt "\n", ##__VA_ARGS__);                   \
    } while (0)
 ...
 */
#define TIME_START(X)                                                          \
    auto X##_START = std::chrono::steady_clock::now(), X##_END = X##_START
 ...
 */
#define TIME_NSEC(X)                                                           \
    std::chrono::duration_cast<std::chrono::nanoseconds>(X##_END - X##_START)  \
        .count()
...
#define TIME_NSEC_SHOW(X)                                                      \
    cout << "Func " << #X << " cost : " << TIME_NSEC(X) << " ns " << endl
 ...
 */
#define TIME_USEC(X)                                                           \
    std::chrono::duration_cast<std::chrono::microseconds>(X##_END - X##_START) \
        .count()
...
#define TIME_USEC_SHOW(X)                                                      \
    cout << "Func " << #X << " cost : " << TIME_USEC(X) << " us " << endl
 ...
 */
#define TIME_MSEC(X)                                                           \
    std::chrono::duration_cast<std::chrono::milliseconds>(X##_END - X##_START) \
        .count()
...
#define TIME_MSEC_SHOW(X)                                                      \
    cout << "Func " << #X << " cost : " << TIME_MSEC(X) << " ms " << endl
 ...
 */
#define TIME_SEC(X)                                                            \
    std::chrono::duration_cast<std::chrono::seconds>(X##_END - X##_START)      \
        .count()
...
#define TIME_SEC_SHOW(X)                                                       \
    cout << "Func " << #X << " cost : " << TIME_SEC(X) << " s " << endl
...
#define TIME_MINUTE(X)                                                         \
    std::chrono::duration_cast<std::chrono::minutes>(X##_END - X##_START)      \
        .count()
...
#define TIME_MINUTE_SHOW(X)                                                    \
    cout << "Func " << #X << " cost : " << TIME_MINUTE(X) << " min " << endl
...
#define TIME_HOUR_SHOW(X)                                                      \
    cout << "Func " << #X << " cost : " << TIME_HOUR(X) << " h " << endl
 ...
 */
void *CopyDataToDevice(const void  *data,
                       uint32_t     size,
                       aclrtRunMode curRunMode,
                       MemoryType   memType);
 ...
 *                    others: copy failed
 */
AclLiteError CopyDataToDeviceEx(void        *dest,
                                uint32_t     destSize,
                                const void  *src,
                                uint32_t     srcSize,
 ...
 * @return void* The dest memory pointer
 */
void *CopyDataToHost(const void  *data,
                     uint32_t     size,
                     aclrtRunMode curRunMode,
                     MemoryType   memType);
 ...
 *                    others: copy failed
 */
AclLiteError CopyDataToHostEx(void        *dest,
                              uint32_t     destSize,
                              const void  *src,
                              uint32_t     srcSize,
                              aclrtRunMode runMode);
 ...
 *                       ACL_MEMCPY_HOST_TO_HOST, // Memory copy from Host to
 * Host ACL_MEMCPY_HOST_TO_DEVICE, // Memory copy from Host to Device
 *                       ACL_MEMCPY_DEVICE_TO_HOST, // Memory copy from Device
 * to Host ACL_MEMCPY_DEVICE_TO_DEVICE, // Memory copy from Device to Device }
 * aclrtMemcpyKind;
 ...
 */
AclLiteError
ReadBinFile(const std::string &filename, void *&data, uint32_t &size);
 ...
 *                    others: read failed
 */
AclLiteError CopyImageToDevice(ImageData   &destImage,
                               ImageData   &srcImage,
                               aclrtRunMode curRunMode,
                               MemoryType   memType);
