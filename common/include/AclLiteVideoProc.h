...
class AclLiteVideoProc
{
  public:
    AclLiteVideoProc();
    AclLiteVideoProc(uint32_t cameraId,
                     uint32_t width = 1280,
                     uint32_t height = 720,
                     uint32_t fps = 15);
    ...
    AclLiteVideoProc(VencConfig &vencConfig, aclrtContext context = nullptr);
    ~AclLiteVideoProc();

    bool IsOpened();
    /**
     * @brief Set the actual value of the corresponding attribute according to
     * the key value
     * @param [in]: key: enumeration type StreamProperty, see the
     * AclLiteVideoCapBase class for details
