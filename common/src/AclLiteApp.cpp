...
AclLiteApp::AclLiteApp() : isReleased_(false), waitEnd_(false) { Init(); }

AclLiteApp::~AclLiteApp() { ReleaseThreads(); }

AclLiteError AclLiteApp::Init()
{
    const uint32_t    msgQueueSize = 256;
    AclLiteThreadMgr *thMgr =
        new AclLiteThreadMgr(nullptr, "main", msgQueueSize);
    threadList_.push_back(thMgr);
    thMgr->SetStatus(THREAD_RUNNING);
    return ACLLITE_OK;
}

int AclLiteApp::CreateAclLiteThread(AclLiteThread *thInst,
                                    const string  &instName,
                                    aclrtContext   context,
                                    aclrtRunMode   runMode,
                                    const uint32_t msgQueueSize)
{
    int instId = CreateAclLiteThreadMgr(
        thInst, instName, context, runMode, msgQueueSize);
...
int AclLiteApp::CreateAclLiteThreadMgr(AclLiteThread *thInst,
                                       const string  &instName,
                                       aclrtContext   context,
                                       aclrtRunMode   runMode,
                                       const uint32_t msgQueueSize)
{
    ...
    AclLiteThreadMgr *thMgr =
        new AclLiteThreadMgr(thInst, instName, msgQueueSize);
    threadList_.push_back(thMgr);
    ...
    for (size_t i = 0; i < threadParamTbl.size(); i++)
    {
        ...
        if (ret != ACLLITE_OK)
        {
            ACLLITE_LOG_ERROR("Create thread %s failed, error %d",
                              threadParamTbl[i].threadInstName.c_str(),
                              ret);
            return ret;
        }
    }
...
AclLiteError AclLiteApp::SendMessage(int dest, int msgId, shared_ptr<void> data)
{
    ...
    {
        ACLLITE_LOG_ERROR("Send message to %d failed for thread not exist",
                          dest);
...
void AclLiteApp::Wait(AclLiteMsgProcess msgProcess, void *param)
{
    ...
    while (true)
    {
        ...
        {
            ACLLITE_LOG_ERROR(
                "AclLite app exit for message %d process error:%d",
                msg->msgId,
                ret);
            break;
        }
...
void AclLiteApp::Exit() { ReleaseThreads(); }
...
AclLiteApp &CreateAclLiteAppInstance() { return AclLiteApp::GetInstance(); }

AclLiteApp &GetAclLiteAppInstance() { return AclLiteApp::GetInstance(); }
