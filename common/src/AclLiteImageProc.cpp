...
AclLiteImageProc::AclLiteImageProc()
    : isReleased_(false), stream_(nullptr), dvppChannelDesc_(nullptr),
      isInitOk_(false)
{
}

AclLiteImageProc::~AclLiteImageProc() { DestroyResource(); }
...
AclLiteError AclLiteImageProc::Init(string mode)
{
        ...
        mode != "")
    {
        ...
        if (aclRet != ACL_SUCCESS)
        {
            ACLLITE_LOG_ERROR("acldvppCreateChannel failed, aclRet = %d",
                              aclRet);
...
// CropAndPasteHelper
AclLiteError AclLiteImageProc::Crop(ImageData &dest,
                                    ImageData &src,
                                    uint32_t   ltHorz,
                                    uint32_t   ltVert,
                                    uint32_t   rbHorz,
                                    uint32_t   rbVert)
{
    CropAndPasteHelper crop(
        stream_, dvppChannelDesc_, ltHorz, ltVert, rbHorz, rbVert);
    return crop.Process(dest, src);
}

AclLiteError AclLiteImageProc::CropPaste(ImageData &dest,
                                         ImageData &src,
                                         uint32_t   width,
                                         uint32_t   height,
                                         uint32_t   ltHorz,
                                         ...
                                         uint32_t   rbVert)
{
    CropAndPasteHelper crop(stream_,
                            dvppChannelDesc_,
                            width,
                            height,
                            ltHorz,
...
AclLiteError AclLiteImageProc::ProportionPaste(ImageData &dest,
                                               ImageData &src,
                                               uint32_t   ltHorz,
                                               uint32_t   ltVert,
                                               uint32_t   rbHorz,
...
AclLiteError AclLiteImageProc::ProportionPasteCenter(ImageData &dest,
                                                     ImageData &src,
                                                     uint32_t   width,
                                                     uint32_t   height)
{
    CropAndPasteHelper crop(stream_, dvppChannelDesc_, 0, 0, width, height);
