...
AclLiteModel::AclLiteModel()
    : loadFlag_(false), isReleased_(false), modelId_(0), outputsNum_(0),
      modelMemSize_(0), modelWorkSize_(0), modelWeightSize_(0),
      modelMemPtr_(nullptr), modelWorkPtr_(nullptr), modelWeightPtr_(nullptr),
      modelDesc_(nullptr), input_(nullptr), output_(nullptr), modelPath_("")
{
}
...
AclLiteModel::AclLiteModel(const string &modelPath)
    : loadFlag_(false), isReleased_(false), modelId_(0), outputsNum_(0),
      modelMemSize_(0), modelWorkSize_(0), modelWeightSize_(0),
      modelMemPtr_(nullptr), modelWorkPtr_(nullptr), modelWeightPtr_(nullptr),
      modelDesc_(nullptr), input_(nullptr), output_(nullptr),
...
AclLiteModel::AclLiteModel(void *modelAddr, size_t modelSize)
    : loadFlag_(false), isReleased_(false), modelId_(0), outputsNum_(0),
      modelMemSize_(modelSize), modelWorkSize_(0), modelWeightSize_(0),
      modelMemPtr_(modelAddr), modelWorkPtr_(nullptr), modelWeightPtr_(nullptr),
      modelDesc_(nullptr), input_(nullptr), output_(nullptr), modelPath_("")
{
}

AclLiteModel::~AclLiteModel() { DestroyResource(); }
...
AclLiteError AclLiteModel::LoadModelFromFile(const string &modelPath)
{
    ...
    {
        ACLLITE_LOG_ERROR(
            "Load model(%s) from file return %d", modelPath.c_str(), ret);
...
AclLiteError AclLiteModel::LoadModelFromMem()
{
    if (loadFlag_)
    {
        ACLLITE_LOG_ERROR("Model is loaded already, address : %p",
                          modelMemPtr_);
        return ACLLITE_ERROR_LOAD_MODEL_REPEATED;
    }
    ...
    {
        ACLLITE_LOG_ERROR(
            "Load model from : %p failed, return %d", modelMemPtr_, ret);
        return ACLLITE_ERROR_LOAD_MODEL;
    }
...
AclLiteError AclLiteModel::CreateInput(void    *input1,
                                       uint32_t input1size,
                                       void    *input2,
                                       uint32_t input2size)
{
...
AclLiteError AclLiteModel::CreateInput(vector<DataInfo> &inputData)
{
    ...
    size_t dynamicIdx = 0;
    auto   ret = aclmdlGetInputIndexByName(
        modelDesc_, ACL_DYNAMIC_TENSOR_NAME, &dynamicIdx);
    if ((ret == ACL_SUCCESS) && (dynamicIdx == (dataNum - 1)))
    {
        ...
        if (ret != ACL_SUCCESS)
        {
            ACLLITE_LOG_ERROR("malloc device memory failed, errorCode = %d.",
    ...
    for (uint32_t i = 0; i < inputData.size(); i++)
    {
        ...
        AclLiteError atlRet =
            AddDatasetBuffer(input_, inputData[i].data, inputData[i].size);
        if (atlRet != ACLLITE_OK)
        {
            ACLLITE_LOG_ERROR("Create input failed for "
                              "add dataset buffer error %d",
                              atlRet);
...
AclLiteError AclLiteModel::CreateOutput()
{
    ...
    for (size_t i = 0; i < outputsNum_; ++i)
    {
        ...
        aclError ret =
            aclrtMalloc(&outputBuffer, bufSize, ACL_MEM_MALLOC_NORMAL_ONLY);
        if (ret != ACL_SUCCESS)
        {
            ACLLITE_LOG_ERROR("Create output failed for malloc "
                              "device failed, size %d",
                              (int)bufSize);
...
AclLiteError AclLiteModel::SetDynamicBatchSize(uint64_t batchSize)
{
    size_t   index;
    aclError ret =
        aclmdlGetInputIndexByName(modelDesc_, ACL_DYNAMIC_TENSOR_NAME, &index);
    if (ret != ACL_SUCCESS)
    {
...
AclLiteError AclLiteModel::ExecuteV2(vector<InferenceOutput> &inferOutputs)
{
    ...
    {
        ACLLITE_LOG_ERROR(
            "Execute model(%s) error:%d", modelPath_.c_str(), ret);
...
AclLiteError
AclLiteModel::GetOutputItem(InferenceOutput &out, uint32_t idx, bool isDevice)
{
    ...
    if (isDevice)
    {
        ...
        {
            ACLLITE_LOG_ERROR(
                "Update DataBuffer %d data for model output failed", idx);
            return ACLLITE_ERROR_MALLOC_DEVICE;
        }
    }
    else
    {
        void *data =
            CopyDataToHost(dataBufferDev, bufferSize, runMode_, MEMORY_NORMAL);
        if (data == nullptr)
        {
...
AclLiteError
AclLiteModel::GetModelOutputInfo(vector<ModelOutputInfo> &modelOutputInfo)
{
    for (size_t i = 0; i < outputsNum_; ++i)
    {
