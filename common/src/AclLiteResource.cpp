...
AclLiteResource::AclLiteResource()
    : isReleased_(false), useDefaultCtx_(true), deviceId_(0),
...
AclLiteResource::AclLiteResource(int32_t       devId,
                                 ...
                                 bool          useDefaultCtx)
    : isReleased_(false), useDefaultCtx_(useDefaultCtx), deviceId_(devId),
      runMode_(ACL_HOST), context_(nullptr), aclConfig_(aclConfigPath)
{
}

AclLiteResource::~AclLiteResource() { Release(); }
...
AclLiteError AclLiteResource::Init()
{
    ...
    {
        ACLLITE_LOG_ERROR(
            "Acl open device %d failed, errorCode is : %d", deviceId_, ret);
        return ACLLITE_ERROR;
    }
    ...
    if (useDefaultCtx_)
    {
        ...
        {
            ACLLITE_LOG_ERROR(
                "Get current acl context failed, errorCode is : %d", ret);
    ...
    else
    {
        ...
        if (ret != ACL_SUCCESS)
        {
            ACLLITE_LOG_ERROR("Create acl context failed, errorCode is : %d",
                              ret);
