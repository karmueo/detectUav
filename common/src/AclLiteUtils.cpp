...
void GetAllFiles(const string &pathList, vector<string> &fileVec)
{
    ...
    for (string everyPath : pathVec)
    {
        ...
        {
            ACLLITE_LOG_ERROR(
                "Failed to deal path=%s. Reason: not exist or can not access.",
                everyPath.c_str());
            continue;
        }
...
void *MallocMemory(uint32_t dataSize, MemoryType memType)
{
    ...
    {
        ACLLITE_LOG_ERROR(
            "Malloc memory failed, type: %d, errorno:%d", memType, aclRet);
        return nullptr;
    }
    return buffer;
}
...
aclrtMemcpyKind
GetCopyPolicy(aclrtRunMode srcDev, CopyDirection direct, MemoryType memType)
{
...
AclLiteError CopyImageToLocal(ImageData   &destImage,
                              ...
                              aclrtRunMode curRunMode)
{
    void *data = CopyDataToHost(
        srcImage.data.get(), srcImage.size, curRunMode, MEMORY_NORMAL);
    if (data == nullptr)
    {
...
AclLiteError CopyImageToDevice(ImageData   &destImage,
                               ...
                               MemoryType   memType)
{
    void *data = CopyDataToDevice(
        srcImage.data.get(), srcImage.size, curRunMode, memType);
    if (data == nullptr)
    {
        return ACLLITE_ERROR_COPY_DATA;
    }
...
AclLiteError ReadPng(ImageData &image, const std::string &fileName)
{
    ...
    int32_t ch = 0;
    acldvppPngGetImageInfo(buf, size, &(image.width), &(image.height), &ch);
    if (image.width == 0 || image.height == 0)
    {
...
void SaveBinFile(const string &filename, const void *data, uint32_t size)
{
    ...
    {
        ACLLITE_LOG_ERROR("Save file %s failed for open error",
                          filename.c_str());
        return;
    }
...
bool AnalyseLine(const string &line, string &key, string &value)
{
    ...
    string new_line =
        line.substr(startPos, startPos + 1 - endPos); // delete comment
