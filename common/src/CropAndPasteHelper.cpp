...
CropAndPasteHelper::CropAndPasteHelper(aclrtStream        &stream,
    ...
    : stream_(stream), vpcOutBufferDev_(nullptr), cropArea_(nullptr),
      pasteArea_(nullptr), vpcInputDesc_(nullptr), vpcOutputDesc_(nullptr),
...
CropAndPasteHelper::~CropAndPasteHelper() { DestroyCropAndPasteResource(); }

AclLiteError
CropAndPasteHelper::InitCropAndPasteInputDesc(ImageData &inputImage)
{
...
AclLiteError CropAndPasteHelper::InitCropAndPasteOutputDesc()
{
    ...
    vpcOutBufferSize_ = YUV420SP_SIZE(cropOutWidthStride, cropOutHeightStride);
...
AclLiteError CropAndPasteHelper::Process(ImageData &cropedImage,
                                         ImageData &srcImage)
{
    ...
    cropArea_ = acldvppCreateRoiConfig(
        cropLeftOffset, cropRightOffset, cropTopOffset, cropBottomOffset);
    ...
    pasteArea_ = acldvppCreateRoiConfig(
        pasteLeftOffset, pasteRightOffset, pasteTopOffset, pasteBottomOffset);
    ...
    // crop and patse pic
    aclError aclRet = acldvppVpcCropAndPasteAsync(dvppChannelDesc_,
                                                  vpcInputDesc_,
                                                  vpcOutputDesc_,
                                                  cropArea_,
                                                  pasteArea_,
    ...
    if (aclRet != ACL_SUCCESS)
    {
        ACLLITE_LOG_ERROR("acldvppVpcCropAndPasteAsync failed, aclRet = %d",
                          aclRet);
        return ACLLITE_ERROR;
    }
    ...
    {
        ACLLITE_LOG_ERROR(
            "crop and paste aclrtSynchronizeStream failed, aclRet = %d",
            aclRet);
        return ACLLITE_ERROR;
    }
...
AclLiteError CropAndPasteHelper::ProcessCropPaste(ImageData &cropedImage,
                                                  ImageData &srcImage)
{
    if (ACLLITE_OK != InitCropAndPasteResource(srcImage))
    {
    ...
    uint32_t pasteTopOffset = 0;
    // must odd
    uint32_t pasteRightOffset =
        (((pasteLeftOffset + size_.width) >> 1) << 1) - 1;
    // must odd
    uint32_t pasteBottomOffset =
        (((pasteTopOffset + size_.height) >> 1) << 1) - 1;
...
AclLiteError CropAndPasteHelper::ProportionProcess(ImageData &resizedImage,
                                                   ImageData &srcImage)
{
    if (ACLLITE_OK != InitCropAndPasteResource(srcImage))
    {
    ...
    uint32_t cropRightOffset =
        (((cropLeftOffset + originalImageWidth_) >> 1) << 1) - 1;
    // must odd
    uint32_t cropBottomOffset =
        (((cropTopOffset + originalImageHeight_) >> 1) << 1) - 1;
...
AclLiteError
CropAndPasteHelper::ProportionCenterProcess(ImageData &resizedImage,
                                            ImageData &srcImage)
{
    if (ACLLITE_OK != InitCropAndPasteResource(srcImage))
    {
