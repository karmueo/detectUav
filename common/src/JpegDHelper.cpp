...
JpegDHelper::JpegDHelper(aclrtStream        &stream,
                         acldvppChannelDesc *dvppChannelDesc)
    : stream_(stream), decodeOutBufferDev_(nullptr), decodeOutputDesc_(nullptr),
      dvppChannelDesc_(dvppChannelDesc), decodeOutWidthStride_(0),
      decodeOutHeightStride_(0)
{
}

JpegDHelper::~JpegDHelper() { DestroyDecodeResource(); }
...
AclLiteError JpegDHelper::InitDecodeOutputDesc(ImageData &inputImage)
{
    ...
    if (strncmp(socVersion, "Ascend310P3", sizeof("Ascend310P3") - 1) == 0)
    {
        ...
        decodeOutWidthStride_ =
            ALIGN_UP64(inputImage.width); // 64-byte alignment
        decodeOutHeightStride_ =
            ALIGN_UP16(inputImage.height); // 16-byte alignment
    }
    else
    {
        ...
        decodeOutWidthStride_ =
            ALIGN_UP128(inputImage.width); // 128-byte alignment
        decodeOutHeightStride_ =
            ALIGN_UP16(inputImage.height); // 16-byte alignment
    }
    ...
    decodeOutBufferSize_ =
        YUV420SP_SIZE(decodeOutWidthStride_, decodeOutHeightStride_);
...
AclLiteError JpegDHelper::Process(ImageData &dest, ImageData &src)
{
    ...
    aclError aclRet =
        acldvppJpegDecodeAsync(dvppChannelDesc_,
