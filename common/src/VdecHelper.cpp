...
VdecHelper::VdecHelper(int             channelId,
                       uint32_t        width,
                       uint32_t        height,
                       int             type,
      ...
      isExit_(false), isReleased_(false)
{
    ...
    {
        ACLLITE_LOG_ERROR("VdecHelper : Get current acl context error:%d",
                          aclRet);
    }
...
AclLiteError VdecHelper::Init()
{
    ...
    int ret = pthread_create(
        &subscribeThreadId_, nullptr, SubscribeReportThreadFunc, (void *)this);
    if (ret)
    {
...
AclLiteError VdecHelper::CreateVdecChannelDesc()
{
    ...
    ret = aclvdecSetChannelDescEnType(
        vdecChannelDesc_, static_cast<acldvppStreamFormat>(enType_));
    if (ret != ACL_SUCCESS)
    {
    ...
    ret = aclvdecSetChannelDescOutPicFormat(
        vdecChannelDesc_, static_cast<acldvppPixelFormat>(format_));
    if (ret != ACL_SUCCESS)
    {
        ACLLITE_LOG_ERROR("Set vdec channel pic format failed, errorno:%d",
                          ret);
...
AclLiteError VdecHelper::CreateInputStreamDesc(shared_ptr<FrameData> frameData)
{
    ...
    if (frameData->isFinished)
    {
        ...
        if (ret != ACL_SUCCESS)
        {
            ACLLITE_LOG_ERROR("Set EOS to input stream desc failed, errorno:%d",
                              ret);
...
AclLiteError VdecHelper::CreateOutputPicDesc(size_t size)
{
    ...
    if (ret != ACL_SUCCESS)
    {
        ACLLITE_LOG_ERROR("Set vdec output pic desc data failed, errorno:%d",
                          ret);
    ...
    if (ret != ACL_SUCCESS)
    {
        ACLLITE_LOG_ERROR("Set vdec output pic widthStride failed, errorno:%d",
                          ret);
    ...
    if (ret != ACL_SUCCESS)
    {
        ACLLITE_LOG_ERROR("Set vdec output pic heightStride failed, errorno:%d",
                          ret);
        return ACLLITE_ERROR;
    }
...
AclLiteError VdecHelper::Process(shared_ptr<FrameData> frameData,
                                 void                 *userData)
{
    // create input desc
    ...
    aclError ret = aclvdecSendFrame(
        vdecChannelDesc_, inputStreamDesc_, outputPicDesc_, nullptr, userData);
    if (ret != ACL_SUCCESS)
    {
...
AclLiteError VdecHelper::VideoParamCheck()
{
    ...
    {
        ACLLITE_LOG_ERROR(
            "video frame width %d is invalid, the legal range is [0, %d]",
            frameWidth_,
            kFrameWidthMax);
    ...
    {
        ACLLITE_LOG_ERROR(
            "video frame height %d is invalid, the legal range is [0, %d]",
            frameHeight_,
            kFrameHeightMax);
        return ACLLITE_ERROR_VDEC_INVALID_PARAM;
    }
