...
int PicToRtsp::AvInit(int picWidth, int picHeight, std::string g_outFile)
{
    ...
    if (avformat_alloc_output_context2(
            &g_fmtCtx, NULL, g_avFormat.c_str(), g_outFile.c_str()) < 0)
    {
...
void PicToRtsp::YuvDataInit()
{
    if (this->g_yuvToRtspFlag == false)
    {
        ...
        g_yuvSize = av_image_get_buffer_size(
            g_codecCtx->pix_fmt, g_codecCtx->width, g_codecCtx->height, 1);

        g_yuvBuf = (uint8_t *)av_malloc(g_yuvSize);

        int ret = av_image_fill_arrays(g_yuvFrame->data,
                                       g_yuvFrame->linesize,
                                       g_yuvBuf,
                                       g_codecCtx->pix_fmt,
                                       g_codecCtx->width,
    ...
    if (avcodec_send_frame(g_codecCtx, g_yuvFrame) >= 0)
    {
        while (avcodec_receive_packet(g_codecCtx, g_pkt) >= 0)
        {
            ...
            av_packet_rescale_ts(
                g_pkt, g_codecCtx->time_base, g_avStream->time_base);
            g_pkt->pos = -1;
...
void PicToRtsp::BgrDataInint()
{
    if (this->g_bgrToRtspFlag == false)
    {
        ...
        g_rgbSize = av_image_get_buffer_size(
            AV_PIX_FMT_BGR24, g_codecCtx->width, g_codecCtx->height, 1);
        g_yuvSize = av_image_get_buffer_size(
            g_codecCtx->pix_fmt, g_codecCtx->width, g_codecCtx->height, 1);
        ...
        int ret = av_image_fill_arrays(g_rgbFrame->data,
                                       g_rgbFrame->linesize,
                                       g_brgBuf,
                                       AV_PIX_FMT_BGR24,
                                       g_codecCtx->width,
        ...
        ret = av_image_fill_arrays(g_yuvFrame->data,
                                   g_yuvFrame->linesize,
                                   g_yuvBuf,
                                   g_codecCtx->pix_fmt,
                                   g_codecCtx->width,
                                   g_codecCtx->height,
                                   1);
        g_imgCtx = sws_getContext(g_codecCtx->width,
                                  g_codecCtx->height,
                                  AV_PIX_FMT_BGR24,
                                  g_codecCtx->width,
                                  g_codecCtx->height,
...
int PicToRtsp::BgrDataToRtsp(void *dataBuf, uint32_t size, uint32_t seq)
{
    if (g_rgbSize != size)
    {
        ACLLITE_LOG_ERROR("bgr data size error, The data size should be %d, "
